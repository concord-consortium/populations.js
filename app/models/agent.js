 ...
 */
let Agent;
const AgentView = require('views/agent-view');
...
module.exports = (Agent = (function() {
  Agent = class Agent {
    static initClass() {
      this.prototype.label = "organism";
      this.prototype.bred = false;
      this.prototype._viewLayer = 1;
  
      this.prototype.isDead = false;
    }
    ...
    */
    reproduce(mate) {
      ...
      const numOffspring = minOffspring + ExtMath.randomInt((1 + maxOffspring) - minOffspring);
    ...
    */
    ...
    resetGeneticTraits(createOrganism){
      ...
      if (this.species.geneticSpecies != null) {
        let trait;
        if (createOrganism) {
          const desired_sex = (this.hasProp('sex') && (this.get('sex') === 'male') ? BioLogica.MALE : BioLogica.FEMALE);
          const allele_set = [];
    ...
    _findOffspringLocation() {
      ...
      const distance = ExtMath.randomValue(minD, maxD);
