...
export default class AgentView {
  ...
  _setSpriteProperties(sprite, image){
    ...
    if (this.imageProperties.initialFlipDirection) {
      d = helpers.ExtMath.normalizeRads(this.agent.get('direction'));
      switch (this.imageProperties.initialFlipDirection) {
        case "left":
          if (-helpers.ExtMath.HALF_PI < d && d < helpers.ExtMath.HALF_PI) { sprite.scale.x *= -1; }
          break;
        case "right":
          if ((-helpers.ExtMath.HALF_PI > d) || (d > helpers.ExtMath.HALF_PI)) { sprite.scale.x *= -1; }
          break;
